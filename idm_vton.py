...
async def get_output_from_idm_vton(garm_img_path, human_img_path, garment_des, file_name):
    # todo: implement
    human_img = Image.open(human_img_path).convert("RGB")
    garment_img = Image.open(garm_img_path).convert("RGB")
    ...
    output_image.save(f"{file_name}")
    
    return "https://i.ibb.co/GRgwBxQ/cropped-flux-output-0.png"

import asyncio
if __name__ == "__main__":
    # run idm_vton image generation
    output_path = "outputs/image.png"
    prompt = "a photo of a woman wearing a short sleeve round neck t-shirt"
    # garm_img_path: Any,
    # human_img_path: Any,
    # garment_des: Any,
    # file_name: Any
    asyncio.run(get_output_from_idm_vton("garment.jpg", "human.jpg", "short sleeve round neck T-shirt", "check.jpg"))